...
final class HomeTabVcRecipeCardLarge: UICollectionViewCell {
    ...
    func configure(with recipe: Recipe) {
        recipeNameLabel.text = recipe.name                             // Set the recipe name
        cookingTimeInMinutesLabel.text = "\(recipe.cookingTime) Min"   // Set the cooking time
        ratingLabel.text = String(format: "%.1f", recipe.rating)       // Set the rating
        foodImage.kf.setImage(with: URL(string: recipe.imageURL))      // Load the image
    }
}
